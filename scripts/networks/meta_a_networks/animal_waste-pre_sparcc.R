...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/animal_waste/")
...
if ("animal_waste" == "gainesville" || "animal_waste" == "chicken_feed") {
    
    metadata_animal_waste = metadata %>%
  dplyr::filter(diet == "animal_waste")

} else {

metadata_animal_waste = metadata %>%
  dplyr::filter(diet_condensed == "animal_waste")

}

samples_kept = rownames(metadata_animal_waste)
...
feature_table_animal_waste = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_animal_waste)
...
taxa_table_animal_waste = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_animal_waste, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_animal_waste))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_animal_waste, 2, colSums(feature_table_animal_waste), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_animal_waste)
...
filtered_feature_table_animal_waste <- feature_table_animal_waste[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_animal_waste), 25)
subsampled_table = filtered_feature_table_animal_waste[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/animal_waste/animal_waste-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/animal_waste/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/animal_waste/animal_waste_pre_sparcc.RData")
