...
load("./sterile_plant_product/sterile_plant_product_pre_sparcc.RData")
...
for (name in names(cor_data_list)) {
  ...
  write.table(vertices_df, file = file.path("./sterile_plant_product/subsample/", name, "vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  write.table(edges_df, file = file.path("./sterile_plant_product/subsample/", name, "edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  graph_df <- data.frame(name = names(graph_attrs), value = unlist(graph_attrs))
  write.table(graph_df, file = file.path("./sterile_plant_product/subsample/", name, "graph_attributes.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  message("TSVs of network attributes have been made.")
  
}

save.image("./sterile_plant_product/subsample/sterile_plant_product_network_gen.RData")
