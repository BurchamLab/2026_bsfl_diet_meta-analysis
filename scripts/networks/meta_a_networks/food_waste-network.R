...
load("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/food_waste_pre_sparcc.RData")


# Load in all correlation and p-value matrices together as lists

base_path = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/sparcc_output/food_waste"
...
for (name in names(cor_data_list)) {
  ...
  tidy.net %<>%
    ...
    mutate(nodeSize = unname(meanSize),
           asv_name = node_map_filtered_taxa$Feature.ID,
  ...
  dir = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/", name)
  
  dir.create(dir, recursive = TRUE)
  
  write.table(vertices_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/", name, "vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  write.table(edges_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/", name, "edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  graph_df <- data.frame(name = names(graph_attrs), value = unlist(graph_attrs))
  write.table(graph_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/", name, "graph_attributes.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  message("TSVs of network attributes have been made.")
  
}

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/food_waste_network_gen.RData")
