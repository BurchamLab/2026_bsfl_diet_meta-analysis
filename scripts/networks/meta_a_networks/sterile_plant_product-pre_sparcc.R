...
metadata_sterile_plant_product = metadata %>%
  dplyr::filter(diet_condensed == "sterile_plant_product")

samples_kept = rownames(metadata_sterile_plant_product)
...
relab_sterile_plant_product = relab[, colnames(relab) %in% samples_kept]

asvs_kept = rownames(relab_sterile_plant_product)
...
taxa_table_sterile_plant_product = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_sterile_plant_product, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(relab_sterile_plant_product))  # Should match the number of rows in tax
...
relab.rel <- sweep(relab_sterile_plant_product, 2, colSums(relab_sterile_plant_product), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_sterile_plant_product)
...
relab_abun_filt <- relab_sterile_plant_product[which(mmo$prev >= (sample_count * 0.2)),]
...
file_path = sprintf("./sterile_plant_product/sterile_plant_product-table_%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

save.image("./sterile_plant_product/sterile_plant_product_pre_sparcc.RData")
