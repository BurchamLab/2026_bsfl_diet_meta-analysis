...
load("./plant_heavy_metal_waste/plant_heavy_metal_waste_pre_sparcc.RData")
...
for (name in names(cor_data_list)) {
  ...
  write.table(vertices_df, file = file.path("./plant_heavy_metal_waste/subsample/", name, "vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  write.table(edges_df, file = file.path("./plant_heavy_metal_waste/subsample/", name, "edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  graph_df <- data.frame(name = names(graph_attrs), value = unlist(graph_attrs))
  write.table(graph_df, file = file.path("./plant_heavy_metal_waste/subsample/", name, "graph_attributes.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  message("TSVs of network attributes have been made.")
  
}

save.image("./plant_heavy_metal_waste/subsample/plant_heavy_metal_waste_network_gen.RData")
