...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/food_waste/")
...
if ("food_waste" == "gainesville" || "food_waste" == "chicken_feed") {
    
    metadata_food_waste = metadata %>%
  dplyr::filter(diet == "food_waste")

} else {

metadata_food_waste = metadata %>%
  dplyr::filter(diet_condensed == "food_waste")

}

samples_kept = rownames(metadata_food_waste)
...
feature_table_food_waste = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_food_waste)
...
taxa_table_food_waste = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_food_waste, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_food_waste))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_food_waste, 2, colSums(feature_table_food_waste), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_food_waste)
...
filtered_feature_table_food_waste <- feature_table_food_waste[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_food_waste), 25)
subsampled_table = filtered_feature_table_food_waste[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/food_waste/food_waste-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/food_waste/food_waste_pre_sparcc.RData")
