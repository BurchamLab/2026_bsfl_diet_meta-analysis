...
load("./plant_heavy_metal_waste/subsample/plant_heavy_metal_waste_network_gen.RData")
...
base_path = "./plant_heavy_metal_waste/subsample"
...
plant_heavy_metal_waste_graph = graph_from_data_frame(edge_file_filtered, directed = FALSE)

node_map_filtered <- node_map %>%
  filter(ASV_number %in% V(plant_heavy_metal_waste_graph)$name)

V(plant_heavy_metal_waste_graph)$asv_id = node_map_filtered$Feature.ID

# 1. Community Detection (Greedy)
comm_greedy <- cluster_fast_greedy(plant_heavy_metal_waste_graph)

# 2. Modularity Calculation
mod_greedy <- modularity(comm_greedy, directed = FALSE)

cat("Greedy Modularity - Plant Waste with Heavy Metals Diet:", mod_greedy)

# 3. Centrality Analysis
# Calculate betweenness centrality to identify hubs
betweenness_values <- betweenness(plant_heavy_metal_waste_graph, normalized = TRUE, directed = FALSE)
V(plant_heavy_metal_waste_graph)$betweenness <- betweenness_values

# Extract top 5 hub nodes
top_hubs <- V(plant_heavy_metal_waste_graph)[order(-V(plant_heavy_metal_waste_graph)$betweenness)][1:5]$name

# Calculate betweenness centrality to identify edges
edge_betweenness_values <- edge_betweenness(plant_heavy_metal_waste_graph, directed = FALSE)
E(plant_heavy_metal_waste_graph)$edge_betweenness <- edge_betweenness_values

# Extract top 5 central edges
top_edges <- E(plant_heavy_metal_waste_graph)[order(-E(plant_heavy_metal_waste_graph)$edge_betweenness)][1:5]
edge_names <- sapply(top_edges, function(edge) paste(ends(plant_heavy_metal_waste_graph, edge), collapse = "--"))

cat("Central Nodes in Plant Waste with Heavy Metals Diet:", top_hubs, "\n")
cat("Central Edges in Plant Waste with Heavy Metals Diet:", edge_names, "\n")

# 4. Node Impact Analysis
impact <- node_impact_analysis(plant_heavy_metal_waste_graph)
...
cumulative_resilience <- resilience_test(plant_heavy_metal_waste_graph, impact, metric = "Modularity_Loss", cumulative = TRUE)
cumulative_resilience_random <- resilience_test(plant_heavy_metal_waste_graph, impact, metric = "random", cumulative = TRUE)
...
ggplot(cumulative_resilience, aes(x = Step, y = Components)) +
  ...
  theme_minimal()
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_components.pdf", device = "pdf", height = 8, width = 10, units = "in")

ggplot(cumulative_resilience_random, aes(x = Step, y = Components)) +
  geom_line(color = "blue") +
  labs(title = "Number of Components During Node Removal", x = "Step (Node Removal)", y = "Number of Components") +
  theme_minimal()
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_components_random.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot(cumulative_resilience, aes(x = Step, y = Avg_Path_Length)) +
  ...
  theme_minimal()
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_avg_path_length.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot(cumulative_resilience_random, aes(x = Step, y = Avg_Path_Length)) +
  ...
  labs(title = "Average Path Length During Node Removal", x = "Step (Node Removal)", y = "Average Path Length") +
  theme_minimal()
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_avg_path_length_random.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot(cumulative_resilience, aes(x = Step, y = Clustering_Coeff)) +
  ...
  theme_minimal()
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_global_clust_coef.pdf", device = "pdf", height = 8, width = 10, units = "in")

ggplot(cumulative_resilience_random, aes(x = Step, y = Clustering_Coeff)) +
  geom_line(color = "green") +
  labs(title = "Global Clustering Coefficient During Node Removal", x = "Step (Node Removal)", y = "Clustering Coefficient") +
  theme_minimal()
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_global_clust_coef_random.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
robustness_results_degree <- calculate_robustness_index(plant_heavy_metal_waste_graph, strategy = "degree", method = "linear")
robustness_results_betweenness <- calculate_robustness_index(plant_heavy_metal_waste_graph, strategy = "betweenness", method = "linear")
...
robustness_rand_results <- calculate_robustness_index(plant_heavy_metal_waste_graph, strategy = "random", bootstrap = 100, method = "linear")

# View the robustness index
cat("Robustness AUC - Plant Waste with Heavy Metals Diet (degree):", robustness_results_degree$Robustness_Index, "\n")
cat("Robustness AUC- Plant Waste with Heavy Metals Diet (betweenness):", robustness_results_betweenness$Robustness_Index, "\n")
...
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_robustness_analysis.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot() +
  ...
  labs(
    title = paste0("Random Removal Robustness Analysis Plant Waste with Heavy Metals Diet (Mean AUC: ", round(mean_auc, 4), ")"),
...
ggsave(filename = "./plant_heavy_metal_waste/plant_heavy_metal_waste_waste_random_removal_robustness.pdf", device = "pdf", width = 10, height = 8, units = "in")
...
V(plant_heavy_metal_waste_graph)$color <- community_colors[memberships]

# Combine and collapse all of the generated feature tables

base_path = "./plant_heavy_metal_waste/"
# base_path = "/Users/burchamlab/Library/CloudStorage/OneDrive-UniversityofTennessee/UTK/Research/Code/bsf_metaa/SparCC-master/plant_heavy_metal_waste/subsample/"
...
matched_t_cft_study <- t_cft_study[match(V(plant_heavy_metal_waste_graph)$asv_id, rownames(t_cft_study)), , drop = FALSE]
V(plant_heavy_metal_waste_graph)$study_pie <- lapply(asplit(matched_t_cft_study, 1), as.numeric)
...
set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_study_network.pdf", width = 10, height = 8)

layout = layout_nicely(plant_heavy_metal_waste_graph)
layout_expanded = layout * 5

plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.shape = "pie",
     vertex.pie = V(plant_heavy_metal_waste_graph)$study_pie,
     vertex.pie.color = list(study_colors),
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - Study Level"
)
...
collasped_ft_diet = merged_table_diet %>%
  group_by(plant_heavy_metal_waste_type) %>%
...
matched_t_cft_diet <- t_cft_diet[match(V(plant_heavy_metal_waste_graph)$asv_id, rownames(t_cft_diet)), , drop = FALSE]
V(plant_heavy_metal_waste_graph)$diet_pie <- lapply(asplit(matched_t_cft_diet, 1), as.numeric)
...
set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_diet_network.pdf", width = 10, height = 8)
...
plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.shape = "pie",
     vertex.pie = V(plant_heavy_metal_waste_graph)$diet_pie,
     vertex.pie.color = list(diet_colors),
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - Diet Level"
)
...
set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_cluster_network.pdf", width = 8, height = 8)

layout = layout_nicely(plant_heavy_metal_waste_graph)
layout_expanded = layout * 5

plot(comm_greedy, plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.size = ifelse(V(plant_heavy_metal_waste_graph)$name %in% top_hubs, 10, 3),  # Hubs larger
     vertex.label = ifelse(V(plant_heavy_metal_waste_graph)$name %in% top_hubs, V(plant_heavy_metal_waste_graph)$name, NA),  # Only label hubs
     ...
     col = V(plant_heavy_metal_waste_graph)$color,  # Explicitly set node colors
     ...
     main = "plant_heavy_metal_waste Waste Network - Clusters & Top Hubs Highlighted",
     edge.color = "gray")
...
V(plant_heavy_metal_waste_graph)$asv_name <- V(tidy.net)$asv_name[match(V(plant_heavy_metal_waste_graph)$name, V(tidy.net)$name)]

ASV_names <- unique(V(plant_heavy_metal_waste_graph)$asv_name)
...
V(plant_heavy_metal_waste_graph)$ASV_color <- ASV_colors[V(plant_heavy_metal_waste_graph)$asv_name]

legend_labels <- ASV_list

set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_ASV_network.pdf", width = 18, height = 10)
...
plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.color = V(plant_heavy_metal_waste_graph)$ASV_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - ASV Level"
)
...
V(plant_heavy_metal_waste_graph)$genus_name = tax$Genus[match(V(plant_heavy_metal_waste_graph)$asv_id, rownames(tax))]

genus_names <- unique(V(plant_heavy_metal_waste_graph)$genus_name)
...
V(plant_heavy_metal_waste_graph)$genus_color <- Genus_colors[V(plant_heavy_metal_waste_graph)$genus_name]

legend_labels <- genus_list

set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_genus_network.pdf", width = 14, height = 10)
...
plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.color = V(plant_heavy_metal_waste_graph)$genus_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - Genus Level"
)
...
V(plant_heavy_metal_waste_graph)$family_name = tax$Family[match(V(plant_heavy_metal_waste_graph)$asv_id, rownames(tax))]

family_names <- unique(V(plant_heavy_metal_waste_graph)$family_name)
...
V(plant_heavy_metal_waste_graph)$family_color <- Family_colors[V(plant_heavy_metal_waste_graph)$family_name]

legend_labels <- family_list

set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_family_network.pdf", width = 14, height = 10)
...
plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.color = V(plant_heavy_metal_waste_graph)$family_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - Family Level"
)
...
V(plant_heavy_metal_waste_graph)$order_name = tax$Order[match(V(plant_heavy_metal_waste_graph)$asv_id, rownames(tax))]

order_names <- unique(V(plant_heavy_metal_waste_graph)$order_name)
...
V(plant_heavy_metal_waste_graph)$order_color <- Order_colors[V(plant_heavy_metal_waste_graph)$order_name]

legend_labels <- order_list

set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_order_network.pdf", width = 14, height = 10)
...
plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.color = V(plant_heavy_metal_waste_graph)$order_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - Order Level"
)
...
V(plant_heavy_metal_waste_graph)$class_name = tax$Class[match(V(plant_heavy_metal_waste_graph)$asv_id, rownames(tax))]

class_names <- unique(V(plant_heavy_metal_waste_graph)$class_name)
...
V(plant_heavy_metal_waste_graph)$class_color <- Class_colors[V(plant_heavy_metal_waste_graph)$class_name]

legend_labels <- class_list

set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_class_network.pdf", width = 14, height = 10)
...
plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.color = V(plant_heavy_metal_waste_graph)$class_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - Class Level"
)
...
V(plant_heavy_metal_waste_graph)$phylum_name = tax$Phylum[match(V(plant_heavy_metal_waste_graph)$asv_id, rownames(tax))]

phylum_names <- unique(V(plant_heavy_metal_waste_graph)$phylum_name)
...
V(plant_heavy_metal_waste_graph)$phylum_color <- Phylum_colors[V(plant_heavy_metal_waste_graph)$phylum_name]

legend_labels <- phylum_list

set.seed(76)
pdf("./plant_heavy_metal_waste/plant_heavy_metal_waste_phylum_network.pdf", width = 14, height = 10)
...
plot(plant_heavy_metal_waste_graph,
     layout = layout_expanded,
     vertex.color = V(plant_heavy_metal_waste_graph)$phylum_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "Plant Waste with Heavy Metals Network - Phylum Level"
)
...
# Vertices
vertices_df <- igraph::as_data_frame(plant_heavy_metal_waste_graph, what = "vertices")
...
write.table(vertices_df_clean, file = file.path("./plant_heavy_metal_waste/plant_heavy_metal_waste_vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)


# Edges
edges_df <- igraph::as_data_frame(plant_heavy_metal_waste_graph, what = "edges")
write.table(edges_df, file = file.path("./plant_heavy_metal_waste/plant_heavy_metal_waste_edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
