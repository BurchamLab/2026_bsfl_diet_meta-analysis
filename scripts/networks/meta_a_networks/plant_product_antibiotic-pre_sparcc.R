...
metadata_plant_product_antibiotic = metadata %>%
  dplyr::filter(diet_condensed == "plant_product_antibiotic")

samples_kept = rownames(metadata_plant_product_antibiotic)
...
relab_plant_product_antibiotic = relab[, colnames(relab) %in% samples_kept]

asvs_kept = rownames(relab_plant_product_antibiotic)
...
taxa_table_plant_product_antibiotic = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_plant_product_antibiotic, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(relab_plant_product_antibiotic))  # Should match the number of rows in tax
...
relab.rel <- sweep(relab_plant_product_antibiotic, 2, colSums(relab_plant_product_antibiotic), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_plant_product_antibiotic)
...
relab_abun_filt <- relab_plant_product_antibiotic[which(mmo$prev >= (sample_count * 0.2)),]
...
file_path = sprintf("./plant_product_antibiotic/plant_product_antibiotic-table_%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

save.image("./plant_product_antibiotic/plant_product_antibiotic_pre_sparcc.RData")
