...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/plant_product_plastic/")
...
if ("plant_product_plastic" == "gainesville" || "plant_product_plastic" == "chicken_feed") {
    
    metadata_plant_product_plastic = metadata %>%
  dplyr::filter(diet == "plant_product_plastic")

} else {

metadata_plant_product_plastic = metadata %>%
  dplyr::filter(diet_condensed == "plant_product_plastic")

}

samples_kept = rownames(metadata_plant_product_plastic)
...
feature_table_plant_product_plastic = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_plant_product_plastic)
...
taxa_table_plant_product_plastic = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_plant_product_plastic, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_plant_product_plastic))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_plant_product_plastic, 2, colSums(feature_table_plant_product_plastic), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_plant_product_plastic)
...
filtered_feature_table_plant_product_plastic <- feature_table_plant_product_plastic[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_plant_product_plastic), 25)
subsampled_table = filtered_feature_table_plant_product_plastic[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/plant_product_plastic/plant_product_plastic-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/plant_product_plastic/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/plant_product_plastic/plant_product_plastic_pre_sparcc.RData")
