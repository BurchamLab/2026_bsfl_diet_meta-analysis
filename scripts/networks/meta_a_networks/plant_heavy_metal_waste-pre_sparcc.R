...
metadata_plant_heavy_metal_waste = metadata %>%
  dplyr::filter(diet_condensed == "plant_heavy_metal_waste")

samples_kept = rownames(metadata_plant_heavy_metal_waste)
...
relab_plant_heavy_metal_waste = relab[, colnames(relab) %in% samples_kept]

asvs_kept = rownames(relab_plant_heavy_metal_waste)
...
taxa_table_plant_heavy_metal_waste = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_plant_heavy_metal_waste, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(relab_plant_heavy_metal_waste))  # Should match the number of rows in tax
...
relab.rel <- sweep(relab_plant_heavy_metal_waste, 2, colSums(relab_plant_heavy_metal_waste), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_plant_heavy_metal_waste)
...
relab_abun_filt <- relab_plant_heavy_metal_waste[which(mmo$prev >= (sample_count * 0.2)),]
...
file_path = sprintf("./plant_heavy_metal_waste/plant_heavy_metal_waste-table_%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

save.image("./plant_heavy_metal_waste/plant_heavy_metal_waste_pre_sparcc.RData")
