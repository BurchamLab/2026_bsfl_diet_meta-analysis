...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/gainesville/")
...
if ("gainesville" == "gainesville" || "gainesville" == "chicken_feed") {
    
    metadata_gainesville = metadata %>%
  dplyr::filter(diet == "gainesville")

} else {

metadata_gainesville = metadata %>%
  dplyr::filter(diet_condensed == "gainesville")

}

samples_kept = rownames(metadata_gainesville)
...
feature_table_gainesville = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_gainesville)
...
taxa_table_gainesville = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_gainesville, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_gainesville))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_gainesville, 2, colSums(feature_table_gainesville), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_gainesville)
...
filtered_feature_table_gainesville <- feature_table_gainesville[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_gainesville), 25)
subsampled_table = filtered_feature_table_gainesville[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/gainesville/gainesville-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/gainesville/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/gainesville/gainesville_pre_sparcc.RData")
