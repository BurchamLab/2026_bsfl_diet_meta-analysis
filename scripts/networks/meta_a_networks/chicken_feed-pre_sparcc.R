...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/chicken_feed/")
...
if ("chicken_feed" == "gainesville" || "chicken_feed" == "chicken_feed") {
    
    metadata_chicken_feed = metadata %>%
  dplyr::filter(diet == "chicken_feed")

} else {

metadata_chicken_feed = metadata %>%
  dplyr::filter(diet_condensed == "chicken_feed")

}

samples_kept = rownames(metadata_chicken_feed)
...
feature_table_chicken_feed = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_chicken_feed)
...
taxa_table_chicken_feed = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_chicken_feed, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_chicken_feed))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_chicken_feed, 2, colSums(feature_table_chicken_feed), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_chicken_feed)
...
filtered_feature_table_chicken_feed <- feature_table_chicken_feed[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_chicken_feed), 25)
subsampled_table = filtered_feature_table_chicken_feed[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/chicken_feed/chicken_feed-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_pre_sparcc.RData")
