...
load("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_network_gen.RData")
    ...
    if (cumulative) {
      for (i in seq_along(top_nodes)) {
        ...
        resilience_metrics <- rbind(
          ...
          data.frame(
            Step = i,
            Percent_Removed = i / length(V(chicken_feed_graph)),
...
base_path = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/"
...
chicken_feed_graph = graph_from_data_frame(edge_file_filtered, directed = FALSE)

node_map_filtered <- node_map %>%
  filter(ASV_number %in% V(chicken_feed_graph)$name)

V(chicken_feed_graph)$asv_id = node_map_filtered$Feature.ID

# 1. Community Detection (Greedy)
comm_greedy <- cluster_fast_greedy(chicken_feed_graph)

# 2. Metric Calculation
mod_greedy <- modularity(comm_greedy, directed = FALSE)

cat("Greedy Modularity - chicken_feed Diet:", mod_greedy)

clust_coef = transitivity(chicken_feed_graph, type = "global")

cat("Global Clustering Coefficient - chicken_feed Diet:", clust_coef)

avg_path_len = mean_distance(chicken_feed_graph, directed = FALSE)

cat("Global Average Path Length - chicken_feed Diet:", avg_path_len)

global_betweenness = betweenness(chicken_feed_graph, directed = FALSE, normalized = TRUE)

cat("Mean Global Betweenness - chicken_feed Diet:", mean(global_betweenness))

global_degree = degree(chicken_feed_graph, normalized = TRUE)

cat("Mean Global Degree - chicken_feed Diet:", mean(global_degree))

global_closeness = closeness(chicken_feed_graph, normalized = TRUE)

cat("Mean Global Closeness - chicken_feed Diet:", mean(global_closeness))

global_eigenvector = eigen_centrality(chicken_feed_graph, directed = FALSE)$vector

cat("Mean Global Eigenvector - chicken_feed Diet:", mean(global_eigenvector))

# 3. Centrality Analysis
# Calculate betweenness centrality to identify hubs
betweenness_values <- betweenness(chicken_feed_graph, normalized = TRUE, directed = FALSE)
V(chicken_feed_graph)$betweenness <- betweenness_values

# Extract top 5 hub nodes
top_hubs <- V(chicken_feed_graph)[order(-V(chicken_feed_graph)$betweenness)][1:5]$name

# Calculate betweenness centrality to identify edges
edge_betweenness_values <- edge_betweenness(chicken_feed_graph, directed = FALSE)
E(chicken_feed_graph)$edge_betweenness <- edge_betweenness_values

# Extract top 5 central edges
top_edges <- E(chicken_feed_graph)[order(-E(chicken_feed_graph)$edge_betweenness)][1:5]
edge_names <- sapply(top_edges, function(edge) paste(ends(chicken_feed_graph, edge), collapse = "--"))

cat("Central Nodes in chicken_feed Diet:", top_hubs, "\n")
cat("Central Edges in chicken_feed Diet:", edge_names, "\n")

# 4. Node Impact Analysis
impact <- node_impact_analysis(chicken_feed_graph)
...
cumulative_resilience_mod <- resilience_test(chicken_feed_graph, impact, metric = "Modularity_Loss", cumulative = TRUE, bootstrap = 1)

write.table(cumulative_resilience_mod, file = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/modularity_clust_coef.tsv", row.names = FALSE, sep = "\t")

cumulative_resilience_mod_filt = cumulative_resilience_mod %>%
  filter(is.finite(Clustering_Coeff), is.finite(Percent_Removed))

cumulative_resilience_bet <- resilience_test(chicken_feed_graph, impact, metric = "Betweenness_Loss", cumulative = TRUE, bootstrap = 1)

write.table(cumulative_resilience_bet, file = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/betweenness_clust_coef.tsv", row.names = FALSE, sep = "\t")

cumulative_resilience_bet_filt = cumulative_resilience_bet %>%
  filter(is.finite(Clustering_Coeff), is.finite(Percent_Removed))

cumulative_resilience_close <- resilience_test(chicken_feed_graph, impact, metric = "Closeness_Loss", cumulative = TRUE, bootstrap = 1)

write.table(cumulative_resilience_close, file = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/closeness_clust_coef.tsv", row.names = FALSE, sep = "\t")

cumulative_resilience_close_filt = cumulative_resilience_close %>%
  filter(is.finite(Clustering_Coeff), is.finite(Percent_Removed))

cumulative_resilience_deg <- resilience_test(chicken_feed_graph, impact, metric = "Degree_Loss", cumulative = TRUE, bootstrap = 1)

write.table(cumulative_resilience_deg, file = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/degree_clust_coef.tsv", row.names = FALSE, sep = "\t")

cumulative_resilience_deg_filt = cumulative_resilience_deg %>%
  filter(is.finite(Clustering_Coeff), is.finite(Percent_Removed))

cumulative_resilience_eig <- resilience_test(chicken_feed_graph, impact, metric = "Eigenvector_Loss", cumulative = TRUE, bootstrap = 1)

write.table(cumulative_resilience_eig, file = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/eigenvector_clust_coef.tsv", row.names = FALSE, sep = "\t")

cumulative_resilience_eig_filt = cumulative_resilience_eig %>%
  filter(is.finite(Clustering_Coeff), is.finite(Percent_Removed))

cumulative_resilience_random <- resilience_test(chicken_feed_graph, impact, metric = "random", cumulative = TRUE, bootstrap = 100)

# Calculate AUC for each metric

auc_chicken_feed = data.frame(Diet = "chicken_feed", 
...
write.table(auc_chicken_feed, file = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/metric_auc.tsv", sep = "\t", row.names = FALSE)
...
ci_clust_random = cumulative_resilience_random %>%
  ...
  summarize(
    Mean  = mean(Clustering_Coeff, 0.975, na.rm = TRUE),
    Lower = quantile(Clustering_Coeff, 0.025, na.rm = TRUE),
    Upper = quantile(Clustering_Coeff, 0.975, na.rm = TRUE),
    .groups = "drop"
  )
...
auc_table = valid_traj %>%
  ...
  summarize(
    AUC = {
      x = pick(Percent_Removed)[[1]]
      y = pick(Clustering_Coeff)[[1]]
      x = as.vector(x)
      y = as.vector(y)
      auc(x, y, type = "linear")
    },
...
write.table(auc_table, file = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/random_auc_table.tsv", sep = "\t", row.names = FALSE)
...
random_plot = ggplot() +
  geom_line(
    ...
    aes(x = Percent_Removed, y = Clustering_Coeff, group = Bootstrap),
    color = "gray",
    alpha = 0.4
  ) +
  geom_ribbon(
  ...
  labs(
    title = "chicken_feed - Global Clustering Coefficient",
...
ggsave(
  "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef_random_bootstrap.pdf",
  random_plot,
...
ggplot(cumulative_resilience_bet, aes(x = Percent_Removed, y = Clustering_Coeff)) +
  ...
  theme_minimal()
ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef_betweenness.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot(cumulative_resilience_mod, aes(x = Percent_Removed, y = Clustering_Coeff)) +
  ...
  theme_minimal()
ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef_modularity.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot(cumulative_resilience_deg, aes(x = Percent_Removed, y = Clustering_Coeff)) +
  ...
  theme_minimal()
ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef_degree.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot(cumulative_resilience_eig, aes(x = Percent_Removed, y = Clustering_Coeff)) +
  ...
  theme_minimal()
ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef_eigenvector.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot(cumulative_resilience_close, aes(x = Percent_Removed, y = Clustering_Coeff)) +
  ...
  theme_minimal()
ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef_closeness.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
#   theme_minimal()
# ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_components.pdf", device = "pdf", height = 8, width = 10, units = "in")
# 
# ggplot(cumulative_resilience_random, aes(x = Step, y = Components)) +
#   geom_line(color = "blue") +
#   labs(title = "Number of Components During Node Removal", x = "Step (Node Removal)", y = "Number of Components") +
#   theme_minimal()
# ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_components_random.pdf", device = "pdf", height = 8, width = 10, units = "in")
# 
# 
...
#   theme_minimal()
# ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_avg_path_length.pdf", device = "pdf", height = 8, width = 10, units = "in")
# 
# 
...
#   labs(title = "Average Path Length During Node Removal", x = "Step (Node Removal)", y = "Average Path Length") +
#   theme_minimal()
# ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_avg_path_length_random.pdf", device = "pdf", height = 8, width = 10, units = "in")
# 
# 
...
#   theme_minimal()
# ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef.pdf", device = "pdf", height = 8, width = 10, units = "in")
# 
# ggplot(cumulative_resilience_random, aes(x = Step, y = Clustering_Coeff)) +
#   geom_line(color = "green") +
#   labs(title = "Global Clustering Coefficient During Node Removal", x = "Step (Node Removal)", y = "Clustering Coefficient") +
#   theme_minimal()
# ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_global_clust_coef_random.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
robustness_results_degree <- calculate_robustness_index(chicken_feed_graph, strategy = "degree", method = "linear")
robustness_results_betweenness <- calculate_robustness_index(chicken_feed_graph, strategy = "betweenness", method = "linear")
...
robustness_rand_results <- calculate_robustness_index(chicken_feed_graph, strategy = "random", bootstrap = 100, method = "linear")

# View the robustness index
cat("Robustness AUC - chicken_feed Diet (degree):", robustness_results_degree$Robustness_Index, "\n")
cat("Robustness AUC- chicken_feed Diet (betweenness):", robustness_results_betweenness$Robustness_Index, "\n")
...
ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_robustness_analysis.pdf", device = "pdf", height = 8, width = 10, units = "in")
...
ggplot() +
  ...
  labs(
    title = paste0("Random Removal Robustness Analysis chicken_feed Diet (Mean AUC: ", round(mean_auc, 4), ")"),
...
ggsave(filename = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_waste_random_removal_robustness.pdf", device = "pdf", width = 10, height = 8, units = "in")
...
V(chicken_feed_graph)$color <- community_colors[memberships]

# Combine and collapse all of the generated feature tables

base_path = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/chicken_feed/"
# base_path = "/Users/burchamlab/Library/CloudStorage/OneDrive-UniversityofTennessee/UTK/Research/Code/bsf_metaa/SparCC-master/chicken_feed/subsample/"
...
matched_t_cft_study <- t_cft_study[match(V(chicken_feed_graph)$asv_id, rownames(t_cft_study)), , drop = FALSE]
V(chicken_feed_graph)$study_pie <- lapply(asplit(matched_t_cft_study, 1), as.numeric)
...
set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_study_network.pdf", width = 10, height = 8)

layout = layout_nicely(chicken_feed_graph)
layout_expanded = layout * 5

plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.shape = "pie",
     vertex.pie = V(chicken_feed_graph)$study_pie,
     vertex.pie.color = list(study_colors),
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Network - Study Level"
)
...
if ("chicken_feed" == "plant_product" || "chicken_feed" == "plant_waste") {
...
matched_t_cft_diet <- t_cft_diet[match(V(chicken_feed_graph)$asv_id, rownames(t_cft_diet)), , drop = FALSE]
V(chicken_feed_graph)$diet_pie <- lapply(asplit(matched_t_cft_diet, 1), as.numeric)
...
set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_diet_network.pdf", width = 10, height = 8)
...
plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.shape = "pie",
     vertex.pie = V(chicken_feed_graph)$diet_pie,
     vertex.pie.color = list(diet_colors),
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Network - Diet Level"
)
...
core_nodes = c("094226930fb6d71f02c7b6124105e94b", "313aec37577cdcd2b0ffd21d43eb69ac",
 "3a6b6eaabaee9f111ba41af0866179ef", "49fc1baff761bf9d357844d43cd138ed",
 "75ab6e2bf81c9bdc826128ec90694035", "78056058faedd75d706633c5f55a975f",
 "a0d7da25d08a0eec714a2068c05f244c", "bb21cf70594b50fff63669b576ca18c1",
 "d23fbef2f31d48eda40876cdbc49933a", "ddc522730e2b25acd5021eaa7f43a389",
"fda0746dace4494eb6cfd64b8b7d6036")

set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_cluster_network_core_nodes.pdf", width = 12, height = 8)

layout = layout_nicely(chicken_feed_graph)
layout_expanded = layout * 5

plot(comm_greedy, chicken_feed_graph,
     layout = layout_expanded,
     vertex.size = ifelse(V(chicken_feed_graph)$asv_id %in% core_nodes, 10, 3),  # Core nodes larger
     vertex.label = ifelse(V(chicken_feed_graph)$asv_id %in% core_nodes, V(chicken_feed_graph)$name, NA), # Only label hubs
     vertex.shape = ifelse(V(chicken_feed_graph)$asv_id %in% core_nodes, "square", "circle"), # Make core nodes squares while everything else is a circle
     ...
     col = V(chicken_feed_graph)$color,  # Explicitly set node colors
     ...
     main = "chicken_feed Correlation Network",
     edge.color = "gray")
...
text(
  ...
  labels = sprintf(
    ...
    vcount(chicken_feed_graph),
    ecount(chicken_feed_graph),
    mod_greedy,
    avg_path_len,
    clust_coef,
...
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_cluster_network_top_hubs.pdf", width = 8, height = 8)
...
plot(comm_greedy, chicken_feed_graph,
     layout = layout_expanded,
     vertex.size = ifelse(V(chicken_feed_graph)$name %in% top_hubs, 13, 3),  # Hubs larger
     vertex.label = ifelse(V(chicken_feed_graph)$name %in% top_hubs, V(chicken_feed_graph)$name, NA), # Only label top hubs 
...
V(chicken_feed_graph)$asv_name <- V(tidy.net)$asv_name[match(V(chicken_feed_graph)$name, V(tidy.net)$name)]

ASV_names <- unique(V(chicken_feed_graph)$asv_name)
...
V(chicken_feed_graph)$ASV_color <- ASV_colors[V(chicken_feed_graph)$asv_name]

legend_labels <- ASV_list

set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_ASV_network.pdf", width = 18, height = 10)
...
plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.color = V(chicken_feed_graph)$ASV_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Correlation Network - ASV Level"
)
...
V(chicken_feed_graph)$genus_name = tax$Genus[match(V(chicken_feed_graph)$asv_id, rownames(tax))]

genus_names <- unique(V(chicken_feed_graph)$genus_name)
...
V(chicken_feed_graph)$genus_color <- sep.table_full$Genus_color[match(V(chicken_feed_graph)$genus_name, sep.table_full$Genus)]

legend_labels <- genus_list

set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_genus_network.pdf", width = 14, height = 10)
...
plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.color = V(chicken_feed_graph)$genus_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Correlation Network - Genus Level"
)

legend("topright", legend = legend_labels, fill = V(chicken_feed_graph)$genus_color[match(legend_labels, V(chicken_feed_graph)$genus_name)], border = "black", cex = 0.7)

dev.off()

############### Make a graph that colors by family

V(chicken_feed_graph)$family_name = tax$Family[match(V(chicken_feed_graph)$asv_id, rownames(tax))]

family_names <- unique(V(chicken_feed_graph)$family_name)

# Separate "Unknown" and other phyla
known_families <- sort(family_names[family_names != "Unknown"])
family_list <- c(known_families, "Unknown")  # put "Unknown" last

V(chicken_feed_graph)$family_color <- sep.table_full$Family_color[match(V(chicken_feed_graph)$family_name, sep.table_full$Family)]

legend_labels <- family_list

set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_family_network.pdf", width = 14, height = 10)
...
plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.color = V(chicken_feed_graph)$family_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Correlation Network - Family Level"
)

legend("topright", legend = legend_labels, fill = V(chicken_feed_graph)$family_color[match(legend_labels, V(chicken_feed_graph)$family_name)], border = "black", cex = 0.7)
...
V(chicken_feed_graph)$order_name = tax$Order[match(V(chicken_feed_graph)$asv_id, rownames(tax))]

order_names <- unique(V(chicken_feed_graph)$order_name)

# Separate "Unknown" and other phyla
known_orders <- sort(order_names[order_names != "Unknown"])
order_list <- c(known_orders, "Unknown")  # put "Unknown" last

V(chicken_feed_graph)$order_color <- sep.table_full$Order_color[match(V(chicken_feed_graph)$order_name, sep.table_full$Order)]

legend_labels <- order_list

set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_order_network.pdf", width = 14, height = 10)
...
plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.color = V(chicken_feed_graph)$order_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Correlation Network - Order Level"
)

legend("topright", legend = legend_labels, fill = V(chicken_feed_graph)$order_color[match(legend_labels, V(chicken_feed_graph)$order_name)], border = "black", cex = 0.7)
...
V(chicken_feed_graph)$class_name = tax$Class[match(V(chicken_feed_graph)$asv_id, rownames(tax))]

class_names <- unique(V(chicken_feed_graph)$class_name)

# Separate "Unknown" and other phyla
known_classes <- sort(class_names[class_names != "Unknown"])
class_list <- c(known_classes, "Unknown")  # put "Unknown" last

V(chicken_feed_graph)$class_color <- sep.table_full$Class_color[match(V(chicken_feed_graph)$class_name, sep.table_full$Class)]

legend_labels <- class_list

set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_class_network.pdf", width = 14, height = 10)
...
plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.color = V(chicken_feed_graph)$class_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Correlation Network - Class Level"
)

legend("topright", legend = legend_labels, fill = V(chicken_feed_graph)$class_color[match(legend_labels, V(chicken_feed_graph)$class_name)], border = "black", cex = 0.7)
...
V(chicken_feed_graph)$phylum_name = tax$Phylum[match(V(chicken_feed_graph)$asv_id, rownames(tax))]

phylum_names <- unique(V(chicken_feed_graph)$phylum_name)

# Separate "Unknown" and other phyla
known_phyla <- sort(phylum_names[phylum_names != "Unknown"])
phylum_list <- c(known_phyla, "Unknown")  # put "Unknown" last

V(chicken_feed_graph)$phylum_color <- sep.table_full$Phylum_color[match(V(chicken_feed_graph)$phylum_name, sep.table_full$Phylum)]

legend_labels <- phylum_list

set.seed(76)
pdf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_phylum_network.pdf", width = 14, height = 10)
...
plot(chicken_feed_graph,
     layout = layout_expanded,
     vertex.color = V(chicken_feed_graph)$phylum_color,
     vertex.size = 4,
     vertex.label.color = "white",
     edge.color = "darkgrey",
     vertex.label.cex = 0.35,
     main = "chicken_feed Correlation Network - Phylum Level"
)

legend("topright", legend = legend_labels, fill = V(chicken_feed_graph)$phylum_color[match(legend_labels, V(chicken_feed_graph)$phylum_name)], border = "black", cex = 0.7)
...
# Vertices
vertices_df <- igraph::as_data_frame(chicken_feed_graph, what = "vertices")
...
write.table(vertices_df_clean, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)


# Edges
edges_df <- igraph::as_data_frame(chicken_feed_graph, what = "edges")
write.table(edges_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/chicken_feed/chicken_feed_figures_gen.RData")
