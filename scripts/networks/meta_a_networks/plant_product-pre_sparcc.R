...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/plant_product/")
...
if ("plant_product" == "gainesville" || "plant_product" == "chicken_feed") {
    
    metadata_plant_product = metadata %>%
  dplyr::filter(diet == "plant_product")

} else {

metadata_plant_product = metadata %>%
  dplyr::filter(diet_condensed == "plant_product")

}

samples_kept = rownames(metadata_plant_product)
...
feature_table_plant_product = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_plant_product)
...
taxa_table_plant_product = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_plant_product, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_plant_product))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_plant_product, 2, colSums(feature_table_plant_product), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_plant_product)
...
filtered_feature_table_plant_product <- feature_table_plant_product[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_plant_product), 25)
subsampled_table = filtered_feature_table_plant_product[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/plant_product/plant_product-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/plant_product/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/plant_product/plant_product_pre_sparcc.RData")
