...
load("./plant_product_antibiotic/plant_product_antibiotic_pre_sparcc.RData")
# load("~/Library/CloudStorage/OneDrive-UniversityofTennessee/UTK/Research/Code/bsf_metaa/SparCC-master/plant/batch_effects_test/20_prev_0.1_abun_plant_pre_cor_mat.RData")
...
for (name in names(cor_data_list)) {
  ...
  full_taxonomy = read.table(file = "./full_data/bsf_full_data_taxonomy.txt", sep = "\t", header = TRUE)
  ...
  write.table(vertices_df, file = file.path("./plant_product_antibiotic/subsample/", name, "vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  write.table(edges_df, file = file.path("./plant_product_antibiotic/subsample/", name, "edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  graph_df <- data.frame(name = names(graph_attrs), value = unlist(graph_attrs))
  write.table(graph_df, file = file.path("./plant_product_antibiotic/subsample/", name, "graph_attributes.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  message("TSVs of network attributes have been made.")
  
}

save.image("./plant_product_antibiotic/subsample/plant_product_antibiotic_network_gen.RData")
