...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/mixed_product/")
...
if ("mixed_product" == "gainesville" || "mixed_product" == "chicken_feed") {
    
    metadata_mixed_product = metadata %>%
  dplyr::filter(diet == "mixed_product")

} else {

metadata_mixed_product = metadata %>%
  dplyr::filter(diet_condensed == "mixed_product")

}

samples_kept = rownames(metadata_mixed_product)
...
feature_table_mixed_product = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_mixed_product)
...
taxa_table_mixed_product = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_mixed_product, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_mixed_product))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_mixed_product, 2, colSums(feature_table_mixed_product), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_mixed_product)
...
filtered_feature_table_mixed_product <- feature_table_mixed_product[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_mixed_product), 25)
subsampled_table = filtered_feature_table_mixed_product[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/mixed_product/mixed_product-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/mixed_product/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/mixed_product/mixed_product_pre_sparcc.RData")
