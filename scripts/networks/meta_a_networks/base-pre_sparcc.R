...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/variable/")
...
if ("variable" == "gainesville" || "variable" == "chicken_feed") {
    
    metadata_variable = metadata %>%
  dplyr::filter(diet == "variable")

} else {

metadata_variable = metadata %>%
  dplyr::filter(diet_condensed == "variable")

}

samples_kept = rownames(metadata_variable)
...
feature_table_variable = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_variable)
...
taxa_table_variable = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_variable, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_variable))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_variable, 2, colSums(feature_table_variable), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_variable)
...
filtered_feature_table_variable <- feature_table_variable[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_variable), 25)
subsampled_table = filtered_feature_table_variable[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/feature_tables/variable/variable-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/variable/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/meta_a_networks/variable/variable_pre_sparcc.RData")
