...
load("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/variable/variable_pre_sparcc.RData")


# Load in all correlation and p-value matrices together as lists

base_path = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/sparcc_output/variable"
...
for (name in names(cor_data_list)) {
  ...
  dir = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/variable/", name)
  
  dir.create(dir, recursive = TRUE)
  
  write.table(vertices_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/variable/", name, "vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  write.table(edges_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/variable/", name, "edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  graph_df <- data.frame(name = names(graph_attrs), value = unlist(graph_attrs))
  write.table(graph_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/variable/", name, "graph_attributes.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  message("TSVs of network attributes have been made.")
  
}

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/variable/variable_network_gen.RData")
