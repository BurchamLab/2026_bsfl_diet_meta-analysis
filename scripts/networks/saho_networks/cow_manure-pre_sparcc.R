...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/cow_manure/", recursive = TRUE)
...
if ("cow_manure" == "gainesville" || "cow_manure" == "chicken_feed") {
    
    metadata_cow_manure = metadata %>%
  dplyr::filter(diet == "cow_manure")
...
samples_kept = rownames(metadata_cow_manure)
...
feature_table_cow_manure = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_cow_manure)
...
taxa_table_cow_manure = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_cow_manure, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_cow_manure))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_cow_manure, 2, colSums(feature_table_cow_manure), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_cow_manure)
...
filtered_feature_table_cow_manure <- feature_table_cow_manure[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_cow_manure), 4)
subsampled_table = filtered_feature_table_cow_manure[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/cow_manure/cow_manure-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/cow_manure/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/cow_manure/cow_manure_pre_sparcc.RData")
