...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/bread/", recursive = TRUE)
...
if ("bread" == "gainesville" || "bread" == "chicken_feed") {
    
    metadata_bread = metadata %>%
  dplyr::filter(diet == "bread")

} else {

metadata_bread = metadata %>%
  dplyr::filter(diet_condensed == "bread")

}

samples_kept = rownames(metadata_bread)
...
feature_table_bread = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_bread)
...
taxa_table_bread = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_bread, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_bread))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_bread, 2, colSums(feature_table_bread), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_bread)
...
filtered_feature_table_bread <- feature_table_bread[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_bread), 4)
subsampled_table = filtered_feature_table_bread[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/bread/bread-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/bread/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/bread/bread_pre_sparcc.RData")
