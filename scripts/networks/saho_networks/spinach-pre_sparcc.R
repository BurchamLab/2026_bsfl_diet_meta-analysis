...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/spinach/", recursive = TRUE)
...
if ("spinach" == "gainesville" || "spinach" == "chicken_feed") {
    
    metadata_spinach = metadata %>%
  dplyr::filter(diet == "spinach")
...
samples_kept = rownames(metadata_spinach)
...
feature_table_spinach = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_spinach)
...
taxa_table_spinach = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_spinach, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_spinach))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_spinach, 2, colSums(feature_table_spinach), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_spinach)
...
filtered_feature_table_spinach <- feature_table_spinach[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_spinach), 4)
subsampled_table = filtered_feature_table_spinach[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/spinach/spinach-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/spinach/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/spinach/spinach_pre_sparcc.RData")
