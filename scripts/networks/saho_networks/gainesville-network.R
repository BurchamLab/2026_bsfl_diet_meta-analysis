...
load("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/gainesville/gainesville_pre_sparcc.RData")


# Load in all correlation and p-value matrices together as lists

base_path = "/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/sparcc_output/gainesville"
...
for (name in names(cor_data_list)) {
  ...
  dir = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/gainesville/", name)
  
  dir.create(dir, recursive = TRUE)
  
  write.table(vertices_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/gainesville/", name, "vertices.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  write.table(edges_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/gainesville/", name, "edges.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  graph_df <- data.frame(name = names(graph_attrs), value = unlist(graph_attrs))
  write.table(graph_df, file = file.path("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/gainesville/", name, "graph_attributes.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
  
  message("TSVs of network attributes have been made.")
  
}

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/gainesville/gainesville_network_gen.RData")
