...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/tuna/", recursive = TRUE)
...
if ("tuna" == "gainesville" || "tuna" == "chicken_feed") {
    
    metadata_tuna = metadata %>%
  dplyr::filter(diet == "tuna")

} else {

metadata_tuna = metadata %>%
  dplyr::filter(diet_condensed == "tuna")

}

samples_kept = rownames(metadata_tuna)
...
feature_table_tuna = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_tuna)
...
taxa_table_tuna = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_tuna, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_tuna))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_tuna, 2, colSums(feature_table_tuna), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_tuna)
...
filtered_feature_table_tuna <- feature_table_tuna[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_tuna), 4)
subsampled_table = filtered_feature_table_tuna[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/tuna/tuna-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/tuna/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/tuna/tuna_pre_sparcc.RData")
