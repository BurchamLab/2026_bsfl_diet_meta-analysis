...
dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/coffee_grounds/", recursive = TRUE)
...
if ("coffee_grounds" == "gainesville" || "coffee_grounds" == "chicken_feed") {
    
    metadata_coffee_grounds = metadata %>%
  dplyr::filter(diet == "coffee_grounds")

} else {

metadata_coffee_grounds = metadata %>%
  dplyr::filter(diet_condensed == "coffee_grounds")

}

samples_kept = rownames(metadata_coffee_grounds)
...
feature_table_coffee_grounds = feature_table[, colnames(feature_table) %in% samples_kept]

asvs_kept = rownames(feature_table_coffee_grounds)
...
taxa_table_coffee_grounds = taxa_table[rownames(taxa_table) %in% asvs_kept, ]


### Making a split taxonomy file

sep.table = separate(taxa_table_coffee_grounds, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus"), sep = ";", remove = FALSE)
...
# Check dimensions
print(dim(feature_table_coffee_grounds))  # Should match the number of rows in tax
...
relab.rel <- sweep(feature_table_coffee_grounds, 2, colSums(feature_table_coffee_grounds), FUN="/") * 100 # Calculates relative abundances within each sample
 
############### 0.1% abundance filtering

sample_count = nrow(metadata_coffee_grounds)
...
filtered_feature_table_coffee_grounds <- feature_table_coffee_grounds[filtered_asvs, ]
...
subsamples = sample(colnames(filtered_feature_table_coffee_grounds), 4)
subsampled_table = filtered_feature_table_coffee_grounds[, subsamples]

file_path = sprintf("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/feature_tables/coffee_grounds/coffee_grounds-table-%d.txt", i)

write.table(x = subsampled_table, file = file_path, sep = "\t", col.names = NA, row.names = TRUE)

}

dir.create("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/coffee_grounds/")

save.image("/lustre/isaac24/scratch/rsaho/projects/bsf_metaa/networks/saho_networks/coffee_grounds/coffee_grounds_pre_sparcc.RData")
